    ...
    -- Misc functions
        ...
        function library:get_window_size(properties)
            ...
            if library:is_mobile() then
                return dim2(0, 450, 0, 340)
            end

            return dim2(0, 700, 0, 600)
        end
        ...
        function library:resizify(frame) 
            ...
            Frame.Size = uis.TouchEnabled and dim2(0, 26, 0, 26) or dim2(0, 14, 0, 14)
            ...
            library:connection(uis.InputChanged, function(input, game_event) 
                ...
                if resizing and library:is_pointer_movement(input) then
                    ...
                    local min_pad = uis.TouchEnabled and 40 or 0
                    local min_w = uis.TouchEnabled and floor(og_size.X.Offset * 0.2) or og_size.X.Offset
                    local min_h = uis.TouchEnabled and floor(og_size.Y.Offset * 0.2) or og_size.Y.Offset
        ...
        function library:ensure_size_lock_gui()
            if not uis.TouchEnabled or library.size_lock_gui then
                return
            end
            ...
            local helper_holder = library:create("Frame", {
                ...
                Position = dim2(0, 6, 0, gui_offset + 12);
                Size = dim2(0, 84, 0, 68);
        ...
        local config_holder;
        local style_holder;
        local config_autoload_path = library.directory .. "/autoload.txt"
        ...
        function library:update_style_list()
            ...
            for idx, file in listfiles(library.directory .. "/styling") do
                local name = file:match("([^\\/]+)%.cfg$")
                if name then
                    list[#list + 1] = name
                end
            ...
            style_holder.refresh_options(list)
        end
        ...
        function library:get_style()
            ...
            for theme_name, theme_color in themes.preset do
                Style[theme_name] = theme_color:ToHex()
            end

            return http_service:JSONEncode(Style)
        end

        function library:load_style(style_json)
            local style = http_service:JSONDecode(style_json)

            for theme_name, value in style do
        ...
        function library:save_style(name)
            if not name or name == "" then
                return false, "missing style name"
            end

            writefile(library.directory .. "/styling/" .. name .. ".cfg", library:get_style())
        ...
        function library:load_style_from_name(name)
            ...
            local path = library.directory .. "/styling/" .. name .. ".cfg"
            if not isfile(path) then
                return false, "style file missing"
            end

            library:load_style(readfile(path))
            library:update_style_list()
    ...
    -- Library element functions
        function library:window(properties)
            ...
            local items = cfg.items; do
                ...
                }); library:apply_theme(items[ "main" ], "window", "BackgroundColor3"); items[ "main" ].Position = dim2(0, items[ "main" ].AbsolutePosition.X, 0, items[ "main" ].AbsolutePosition.Y)
        ...
        function library:init_config(window) 
            ...
            local function get_selected_name(text_flag, list_flag)
                local typed = flags[text_flag]
                if typed and typed ~= "" then
                    return typed
                end

                local selected = flags[list_flag]
            ...
            local section = column:section({name = "Configurations", size = 1, default = true, icon = "rbxassetid://139628202576511"})
            ...
            local autoload_label = section:label({name = "autoload: none"})
            section:button({name = "Save", callback = function()
                local name = get_selected_name("config_name_text", "config_name_list")
                if not name then
                ...
                if ok then
                    notifications:create_notification({name = "sportsclub", info = "Saved config: " .. name, lifetime = 3})
                else
                    notify_error("Failed to save config")
                    warn("config_save_error:\n" .. tostring(err))
                end
            end})
            section:button({name = "Load", callback = function()
                ...
                if not isfile(path) then
                    return notify_error("Config not found")
                end
                ...
                if ok then
                    notifications:create_notification({name = "sportsclub", info = "Loaded config: " .. name, lifetime = 3})
                else
                    notify_error("Failed to load config")
                    warn("config_load_error:\n" .. tostring(err))
                end
            end})
            section:button({name = "Delete", callback = function()
                ...
                if ok then
                    notifications:create_notification({name = "sportsclub", info = "Deleted config: " .. name, lifetime = 3})
                else
                    notify_error("Failed to delete config")
                    warn("config_delete_error:\n" .. tostring(err))
                end
            end})
            section:button({name = "Set as Autoload", callback = function()
                local name = flags.config_name_list
                if not name or name == "" then
                    notifications:create_notification({name = "sportsclub", info = "Select a config first", lifetime = 3})
                    return
                end
                ...
                if ok then
                    notifications:create_notification({name = "sportsclub", info = "Autoload set: " .. name, lifetime = 3})
                else
                    notifications:create_notification({name = "sportsclub", info = "Failed to set autoload", lifetime = 6})
                    warn("config_autoload_set_error:\n" .. tostring(err))
                end
            end})
            section:button({name = "Clear Autoload", callback = function()
                local ok, err = pcall(function()
                    if isfile(config_autoload_path) then
                        library:clear_autoload(config_autoload_path)
                        autoload_label.set("autoload: none")
                        notifications:create_notification({name = "sportsclub", info = "Autoload cleared", lifetime = 3})
                    else
                        notifications:create_notification({name = "sportsclub", info = "No autoload set", lifetime = 3})
                    end
                end)

                if not ok then
                    notifications:create_notification({name = "sportsclub", info = "Failed to clear autoload", lifetime = 6})
                    warn("config_autoload_clear_error:\n" .. tostring(err))
                end
            end})
            ...
            style_holder = styling_section:list({options = {"Default"}, callback = function(option) end, flag = "style_name_list"}); library:update_style_list()
            ...
            styling_manage:colorpicker({name = "Menu Accent", flag = "style_accent_color", callback = function(color, alpha) library:update_theme("accent", color) end, color = themes.preset.accent})
            styling_manage:colorpicker({name = "Window Color", flag = "menu_window_color", callback = function(color, alpha) library:update_theme("window", color) end, color = themes.preset.window})
            styling_manage:colorpicker({name = "Section Color", flag = "menu_section_color", callback = function(color, alpha) library:update_theme("section", color) end, color = themes.preset.section})
            styling_manage:colorpicker({name = "Text Color", flag = "menu_text_color", callback = function(color, alpha) library:update_theme("text", color) end, color = themes.preset.text})
            styling_manage:colorpicker({name = "Muted Color", flag = "menu_muted_color", callback = function(color, alpha) library:update_theme("muted", color) end, color = themes.preset.muted})
            styling_manage:button({name = "Save Style", callback = function()
                local name = get_selected_name("style_name_text", "style_name_list")
                if not name then
                    return notify_error("Enter or select a style first")
                end

                local ok, err = library:save_style(name)
                if ok then
                    notifications:create_notification({name = "sportsclub", info = "Saved style: " .. name, lifetime = 3})
                else
                    notify_error("Failed to save style")
                    warn("style_save_error:\n" .. tostring(err))
                end
            end})
            styling_manage:button({name = "Load Style", callback = function()
                ...
                local ok, err = library:load_style_from_name(name)
                if ok then
                    notifications:create_notification({name = "sportsclub", info = "Loaded style: " .. name, lifetime = 3})
                else
                    notify_error("Failed to load style")
                    warn("style_load_error:\n" .. tostring(err))
                end
            end})
            styling_manage:button({name = "Delete Style", callback = function()
                ...
                local ok, err = library:delete_style(name)
                if ok then
                    notifications:create_notification({name = "sportsclub", info = "Deleted style: " .. name, lifetime = 3})
                else
                    notify_error("Failed to delete style")
                    warn("style_delete_error:\n" .. tostring(err))
                end
            end})
            styling_manage:button({name = "Set Style Autoload", callback = function()
                local name = flags.style_name_list
                if not name or name == "" then
                    notifications:create_notification({name = "sportsclub", info = "Select a style first", lifetime = 3})
                    return
                end
                ...
                if ok then
                    notifications:create_notification({name = "sportsclub", info = "Style autoload set: " .. name, lifetime = 3})
                else
                    notifications:create_notification({name = "sportsclub", info = "Failed to set style autoload", lifetime = 6})
                    warn("style_autoload_set_error:\n" .. tostring(err))
                end
            end})
            styling_manage:button({name = "Clear Style Autoload", callback = function()
                local ok, err = pcall(function()
                    if isfile(style_autoload_path) then
                        library:clear_autoload(style_autoload_path)
                        style_autoload_label.set("autoload: none")
                        notifications:create_notification({name = "sportsclub", info = "Style autoload cleared", lifetime = 3})
                    else
                        notifications:create_notification({name = "sportsclub", info = "No style autoload set", lifetime = 3})
                    end
                end)

                if not ok then
                    notifications:create_notification({name = "sportsclub", info = "Failed to clear style autoload", lifetime = 6})
                    warn("style_autoload_clear_error:\n" .. tostring(err))
                end
            end})
            styling_manage:button({name = "Reset Colors", callback = function()
            ...
            library:connection(run.Heartbeat, function(delta_time)
                ...
                local uptime_secs = floor(clock() - start_time)
                local mins = floor(uptime_secs / 60)
            ...
            local server_btns = {
                {"copy jobid", function() setclipboard(game.JobId) notifications:create_notification({name = "sportsclub", info = "Copied job id", lifetime = 2}) end},
                {"copy gameid", function() setclipboard(tostring(game.GameId)) notifications:create_notification({name = "sportsclub", info = "Copied game id", lifetime = 2}) end},
                {"copy join script", function()
                    setclipboard(('game:GetService("TeleportService"):TeleportToPlaceInstance(%d, "%s", game.Players.LocalPlayer)'):format(game.PlaceId, game.JobId))
                    notifications:create_notification({name = "sportsclub", info = "Copied join script", lifetime = 2})
                end},
                {"rejoin", function()
                    notifications:create_notification({name = "sportsclub", info = "Rejoining...", lifetime = 2})
                    task.wait(0.1)
            ...
            for _, b in ipairs(server_btns) do
                section_server:button({name = b[1], callback = b[2]})
            end
            ...
            section_server:button({name = "join new server", callback = function()
                ...
                if not ok or not res or not res.data then
                    notifications:create_notification({name = "Notification", info = "Failed to fetch servers", lifetime = 3})
                    return
                end

                local candidates = {}
                for _, s in ipairs(res.data) do
                    if s.id ~= game.JobId and s.playing >= flags.min_players and s.playing <= flags.max_players then
                        candidates[#candidates + 1] = s
                    end
                end

                if #candidates > 0 then
                    notifications:create_notification({name = "Notification", info = "Joining new server...", lifetime = 2})
                    task.wait(0.1)
                    game.Players.LocalPlayer:Kick()
                    ts:TeleportToPlaceInstance(game.PlaceId, candidates[random(1, #candidates)].id)
                else
                    notifications:create_notification({name = "Notification", info = "No matching servers found", lifetime = 3})
                end
            end})
            ...
            section_server:slider({name = "frames per second", flag = "frames_per_second", min = 30, max = 1000, default = 240, interval = 1, callback = function(v)
                setfpscap(v)
            end})
            ...
            task.spawn(function()
                ...
                if config_name then
                    ...
                    if isfile(config_path) then
                        ...
                        if ok then
                            notifications:create_notification({name = "Notification", info = "Auto loaded config: " .. config_name, lifetime = 3})
                        end
                    end
                end
                ...
                if style_name then
                    local style_path = library.directory .. "/styling/" .. style_name .. ".cfg"
                    if isfile(style_path) then
                        local ok = pcall(function()
                            library:load_style(readfile(style_path))
                        end)
                        if ok then
                            notifications:create_notification({name = "Notification", info = "Auto loaded style: " .. style_name, lifetime = 3})
                        end
                    end
                end
            end)
